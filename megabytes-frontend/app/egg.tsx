...
export default function EggScreen() {
  ...
  const { feedCount, cleanCount, recordFeed, recordClean, advanceStage } = useEvolution();
  ...
  const totalNeeded = 4;
  const totalDone   = Math.min(feedCount + cleanCount, totalNeeded);
  const halfwayDone = totalDone >= 2;
  ...
  const handleFeed = useCallback(async () => {
    ...
    const { evolved } = recordFeed();
    if (evolved) setTimeout(triggerEvolution, 500);
  }, [recordFeed, spawnParticles, triggerEvolution]);
  ...
  const handleClean = useCallback(async () => {
    ...
    const { evolved } = recordClean();
    if (evolved) setTimeout(triggerEvolution, 500);
  }, [recordClean, spawnParticles, triggerEvolution]);
  ...
  return (
    <ImageBackground
      source={require('../assets/backgrounds/bg916.png')}
      style={styles.bg}
      resizeMode="cover"
    >
      ...
      <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
        ...
        <View style={styles.progressWrap}>
          ...
          <Text style={styles.progressLabel}>
            {totalDone >= totalNeeded ? 'HATCHING...' : `INCUBATION ${Math.round((totalDone / totalNeeded) * 100)}%`}
          </Text>
        </View>
        ...
        <View style={styles.statusCard}>
          ...
          <Text style={styles.statusSub}>
            {halfwayDone
              ...
              : 'Warm and stable. Keep caring for it.'}
          </Text>
        </View>
        ...
        <TouchableOpacity style={styles.modalBg} onPress={() => setInspectVisible(false)} activeOpacity={1}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalBody}>
              {halfwayDone
                ? 'This is a Diamond Egg.\n\nSomething stirs inside. The shell pulses faintly with golden light.\n\nIt could hatch at any time.'
                : 'This is a Diamond Egg.\n\nThe shell is warm to the touch. A faint heartbeat can be felt. It is not ready to hatch yet.\n\nKeep caring for it.'}
            </Text>
